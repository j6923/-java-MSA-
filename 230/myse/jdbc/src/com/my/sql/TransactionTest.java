...
//			}
			String sql3 = "UPDATE account SET balance = balance + 10 WHERE no = '777'";  //없는 계조번호 
			String sql5 = "UPDATE account SET balance = balance - 10 WHERE no = '101' "; //있는 계좌번호 
			pstmt = con.prepareStatement(sql5); //101번의 잔액 70원에서 10원을 출금 
			pstmt.executeUpdate();
			...
			pstmt = con.prepareStatement(sql3);  //777번(업슨 계좌)의 10원을 입금  
			...
			String sql6 = "UPDATE account SET balance = balance + 10 WHERE no='101'"; //없는 계좌번호
