...
//		dao = new CustomerDaoList();
		//dao.properties파일구조 키=클래스이름(customer=com.my.customer.dao.CustomerDAOList/ CustomerDAOOracle)  
		//1. dao.properties파일읽기
		Properties env = new Properties();//자바.util패티지에 있다. 
		try {
			env.load(new FileInputStream("dao.properties"));
			//2. customer키에 해당하는 값얻기 
			String className = env.getProperty("customer");
					//3.값을 클래스이름이라 생각하고 런타임다이나믹로드 Class.forName(); 
					
			Class clazz=Class.forName(className);
			//4. 로드된 클래스타입의 객체생성 newInstacne()
			//5. dao에 객체 대입 
